...
func TestDeleteAccountAPI(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account.ID)).Times(1).Return(account, nil)
				store.EXPECT().DeleteAccount(gomock.Any(), gomock.Eq(account.ID)).Times(1).Return(account, nil)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code)
			},
...
func TestUpdateAccountAPI(t *testing.T) {
	...
	updateAmount := util.RandomMoney()
	updatedAccount := account
	updatedBalance := fmt.Sprintf("%f", util.StringToFloat64(account.Balance)+util.StringToFloat64(updateAmount))
	updatedAccount.Balance = updatedBalance
