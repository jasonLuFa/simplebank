...
func (server *Server) deleteAccount(ctx *gin.Context) {
	...
	// check the user have permission to update instead of delete other account
...
func (server *Server) updateAccount(ctx *gin.Context) {
	...
	// check the user have permission to update instead of update other account
	payload := ctx.MustGet(authorizationPayloadKey).(*token.Payload)
	...
	arg := db.UpdateAccountParams{
		ID:      id,
		Balance: fmt.Sprintf("%f", util.StringToFloat64(account.Balance)+util.StringToFloat64(req.Amount)),
	}
